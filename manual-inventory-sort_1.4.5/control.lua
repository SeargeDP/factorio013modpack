...
script.on_event("manual-inventory-sort-opened", function(event) -- sort chest key
	...
	if player.opened and is_sortable(player.opened) then
		local inventory
		local e_type = player.opened.type
		if (e_type == "container" or e_type == "logistic-container") then inventory = player.opened.get_inventory(defines.inventory.chest)
		elseif (e_type == "cargo-wagon") then inventory = player.opened.get_inventory(defines.inventory.cargo_wagon)
		elseif (e_type == "car") then inventory = player.opened.get_inventory(defines.inventory.car_trunk) end
		sort_inventory{player_index = event.player_index, inventory = inventory, force_override = true}
	end
end)
